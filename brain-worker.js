...
function readVarints(bytes,pos,count){const out=new Uint32Array(count);let p=pos;for(let i=0;i<count;i++){let v=0,s=0,b;do{b=bytes[p++];v+=(b&127)*2**s;s+=7}while(b&128);out[i]=v>>>0}return [out,p]}
...
function decodeWeights(buf){
  const bytes=new Uint8Array(buf),d=new DataView(buf);if(d.getUint32(0,false)!==0x464c5957)throw Error('Invalid fly.ai weight header');
  ...
  const degreesAndPos=readVarints(bytes,p,N);const degrees=degreesAndPos[0];p=degreesAndPos[1];indptr=new Uint32Array(N+1);let acc=0;for(let i=0;i<N;i++){indptr[i]=acc;acc+=degrees[i]}indptr[N]=acc;
  ...
  qcodes=new Uint8Array(bytes.slice(p,p+E));
}
...
function buildSamples(){
  aliasSamples={};aliasMask=new Uint16Array(N);for(const a of ALIASES)aliasSamples[a]=[];
  for(let i=0;i<N;i++)for(let g=0;g<ALIASES.length;g++)if(aliasSamples[ALIASES[g]].length<MAX_SAMPLE&&matchAlias(ALIASES[g],i)){aliasSamples[ALIASES[g]].push(i);aliasMask[i]|=1<<g}
}
...
function step(){
  ...
  for(let i=0;i<N;i++)if(V[i]>=THRESHOLD){V[i]=0;next.push(i);const mask=aliasMask[i];for(let g=0;g<ALIASES.length;g++)if(mask&(1<<g))groups[g]++}
  fired=next;tickNo++;self.postMessage({type:'tick',total:next.length,groups:Array.from(groups),tick:tickNo});
}
async function init(){
  self.postMessage({type:'status',message:'Loading fly.ai web connectome metadata…'});const info=await getJson(BRAIN_URL);N=info.neurons;E=info.connections;
  parseMeta(await gunzip(await getBinary(META_URL)));self.postMessage({type:'status',message:`Downloading fly.ai connectome (${info.weights_mb} MB compressed)…`});
  ...
  self.postMessage({type:'status',message:'Decompressing and decoding 166,700-neuron brain…'});decodeWeights(await gunzip(joined.buffer));buildSamples();V=new Float32Array(N);fired=[];
